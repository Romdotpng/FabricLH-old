...
void main() {
    ...
    float distance = (distance0(vertexCoord.xy + offset) + time) / range;
    ...
    if(rainbow) {
        ...
        float distance2 = (distance0(vertexCoord.xy + offset - vec2(800.0, 0.0)) + time) / range;
        float distance3 = (distance0(vertexCoord.xy + offset - vec2(400.0, 400.0)) + time) / range;
        ...
        if(mixAlphas) {
            color = vec4(color.xyz, color.a * vertexColor.a);
        }

        if(originalAlpha/* || (currentColor.a != 0.0 && originalColor.a == 0.0)*/) {
            color = vec4(color.xyz, vertexColor.a);
        }
